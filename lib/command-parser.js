(function() {
  ...
  internals = {
    patterns: {
      insert_replace: /^(?<prefix>.*?)<<<(?<slash>\/?)(?<cmd_name>insert|replace)\x20+((?<cmd_position>below|above)\x20+)?(src\s*=\s*)?(?<cmd_p1>(?:(?:'(?:\\'|[^'])+')|(?:"(?:\\"|[^"])+")|(?:\$[a-zA-Z0-9]+)))>(?<user_eoi>[^>]*)>(?<system_eoi>[^>]*)>(?<suffix>.*?)$/, // insert JS identifier pattern
      publish: /^(?<prefix>.*?)<<<(?<slash>\/?)(?<cmd_name>publish)\x20+((?<disposition>one|enclosed)\x20+)?((?<cmd_position>below|above)\x20+)?(as\s*=\s*)?(?<cmd_p1>(?:(?:'\#(?:\\'|[^'])+')|(?:"\#(?:\\"|[^"])+")))>(?<user_eoi>[^>]*)>(?<system_eoi>[^>]*)>(?<suffix>.*?)$/,
      generic: /^(?<prefix>.*?)<<<(?<slash>\/?)(?<cmd_p1>.*?)>(?<user_eoi>[^>]*)>(?<system_eoi>[^>]*)>(?<suffix>.*?)$/
    }
  };
